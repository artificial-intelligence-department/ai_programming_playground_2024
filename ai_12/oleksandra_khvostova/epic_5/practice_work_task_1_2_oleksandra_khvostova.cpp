...
int main(){
    ...
    if(result1 == Success1) cout << "File was successfully rewrited\n";
    else cout << "Failed to rewrite a file";

    string file1, file2;
    cout << "Enter name of file to copy from(without extension): ";
    getline(cin, file1);
    cout << "Enter name of file in which to copy(without extension): ";
    getline(cin, file2);
    file1 += ".txt";
    file2 += ".txt";
    FileOpResult2 result2 = copy_file(file1.c_str(), file2.c_str());
    if(result2 == Success2) cout << "Content was successfully copied\n";
    else cout << "Failed to copy files";
