...
// Функція для визначення року за старояпонським календарем
std::string getJapaneseYear(int year) {
...
// Функція для визначення знака зодіаку за місяцем
std::string getMonth(const std::string& month) {
...
int main() {
    ...
    int year;
    std::cin >> year;
    std::cout << "Ваш рік за старояпонським календарем, це: " << getJapaneseYear(year) << std::endl;

    std::cout << "Enter a month ";
    std::string month;
    std::cin >> month;
    std::cout << "Ваш знак зодіаку: " << getMonth(month) << std::endl;
