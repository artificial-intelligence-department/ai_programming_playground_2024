...
double square1(const vector<pair<double, double>>& points) {
    int n = points.size();
    if (n < 3) {
    ...
    for (int i = 0; i < n; i++) {
        if (i != a && i != b) {
            double area = square(points[a].first, points[a].second, points[b].first, points[b].second, points[i].first, points[i].second);
            if (area > max_area) {
                max_area = area;
            }
        }
...
int main() {
    ...
    cout << "Edge length: " << distance(0, 0, 5, 0) << endl;
    cout << "Area of a triangle: " << square(0, 0, 5, 0, 3, 4) << endl;
    cout << "Area of the triangle with the longest diagonal: " << square1(points) << endl;
