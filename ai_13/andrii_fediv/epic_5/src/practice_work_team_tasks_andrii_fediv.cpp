...
FileOpResult write_to_file(char *name, char *content) // оскільки файл автоматично створюється то немає сенсу перевіряти чи він створився а також немає як виводити Failure
{
    ...
    file << content;
    if (file.fail()) // true при несерйозних помилках
        return FileOpResult::OutputError;

    file.close();
    if (file.bad()) // true при серйозних помилках
        return FileOpResult::CloseError;

    return FileOpResult::Success;
}

// просто копіює перший файл усередину другого
FileOpResult copyFile(char *file_from, char *file_to)
{
    ...
    if (file1.fail() || file2.fail()) // true при несерйозних помилках
        return FileOpResult::OutputError;

    file1.close();
    file2.close();
    if (file1.bad() || file1.bad()) // true при серйозних помилках
        return FileOpResult::CloseError;

    return FileOpResult::Success;
}

// інтерпретує код результату
