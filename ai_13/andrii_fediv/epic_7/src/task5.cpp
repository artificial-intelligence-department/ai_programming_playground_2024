...
struct type_auto // #19 використано свою структуру даних | для зручності використання об'єднаних даних та декількох методів
{
    string brand;
    int year;
    int price;
    ...
    void saveToBinary(fstream &db) const
    {
        ...
        db.write(reinterpret_cast<const char *>(&brandSize), sizeof(int));// №16 функції роботи з файлами, для того, щоб записати у файл / іменно ця записує розмір наступної змінної
    ...
    void loadFromBinary(fstream &db)
    {
        ...
        if (!db.eof() && db.read(reinterpret_cast<char *>(&brandSize), sizeof(int)))
        {
            ...
            db.read(reinterpret_cast<char *>(&year), sizeof(int));// №17 функції роботи з файлами, для того, щоб зчитати з файлу / іменно ця зчитує розмір наступної змінної
...
private:
    ...
    static int CurrentYear()
    {
        ...
        tm *local = localtime(&now);//#20  використано вказівники | тому що таке значення повертає функція
...
void userInterface(bool error)// #13 використано рекурсію // для повторного виклику функції з іншими вхідними параметрами
{
    type_auto newCar;
    try
    {
